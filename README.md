...
It is recommended to use [uv](https://docs.astral.sh/uv/) for installation, as it is significantly faster than `pip` and handles virtual environments automatically.
...
Then, in the new directory (`cd dkrz-cera/`) install the package and its dependencies via:

```bash
uv sync
```

Alternatively, if you want to add this package as a dependency to another repo, install it directly from GitHub via

```bash
uv add 'git+https://github.com/markusritschel/dkrz-cera.git'
```

> **Note:** If you don't have `uv` installed, follow the [official installation guide](https://docs.astral.sh/uv/getting-started/installation/). Alternatively, all commands above also work with plain `pip install`.
